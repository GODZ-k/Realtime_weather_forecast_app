...
def result(request):
    ...
    city_name = request.POST.get("city", "").strip().lower()  # Use request.POST.get to avoid KeyError
    if not city_name:
        return render(request, "weather_api/results.html", {"city_name": "City name not provided"})

    url = f"http://api.openweathermap.org/data/2.5/forecast?q={city_name}&appid=5b474101163de7cf4d09da4ac876f9bc"
    w_dataset = requests.get(url).json()

    try:
        if w_dataset["cod"] != "200":
            return render(request, "weather_api/results.html", {"city_name": "City not found, check your spelling..."})
        ...
        context = {"city_name": "Data not available"}
